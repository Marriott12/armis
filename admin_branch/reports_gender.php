...
$pageTitle = "Gender Report as at " . date('d-M-Y');
$currentPage = "reports";
...
if ($search !== '') {
    $sql .= " AND (s.gender LIKE ? OR s.svcNo LIKE ? OR s.lName LIKE ? OR s.fName LIKE ? OR r.rankId LIKE ? OR u.unitId LIKE ? OR s.category LIKE ?)";
...
<div class="content-wrapper with-sidebar">
    <div class="container-fluid">
        <div class="main-content">
            <h1 class="section-title mb-4"><i class="fas fa-venus-mars"></i> <?= htmlspecialchars($pageTitle) ?></h1>
            <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                <i class="fas fa-question-circle me-2"></i>
                <span>
                    Filter by gender, unit, rank, category. Use the search box for instant filtering. Export, print, show/hide columns. Double-click row for details.
                </span>
            ...
            <form class="row g-3 mb-4" method="get" action="">
                <div class="col-md-2">
                    <select name="gender" id="genderFilter" class="form-select">
                        ...
                        <?php foreach ($genders as $g): ?>
                            <option value="<?= htmlspecialchars($g) ?>" <?= ($filter_gender == $g) ? 'selected':''?>><?= htmlspecialchars($g) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-3">
                    <input type="text" id="genderSearch" name="search" class="form-control" placeholder="Quick Search..." value="<?=htmlspecialchars($search)?>">
                </div>
                <div class="col-md-1">
            ...
            <div class="table-responsive print-friendly">
                <table class="table table-bordered table-hover align-middle" id="genderTable">
            ...
            <div class="d-flex justify-content-center my-3">
                <nav aria-label="Gender pagination">
...
document.getElementById('exportCSVBtn').addEventListener('click', function() {
    let table = document.getElementById('genderTable');
    ...
    link.download = 'gender_report.csv';
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('genderTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"Gender"});
    XLSX.writeFile(wb, 'gender_report.xlsx');
});
document.getElementById('exportPDFBtn').addEventListener('click', function(){
    ...
    let doc = new jsPDF();
    let startY = 20;
    doc.text("Gender Report", 14, startY);
    rows.forEach(function(row, idx){
        doc.text(row.join(" | "), 14, startY + 8 + idx*8);
    });
    doc.save("gender_report.pdf");
});
document.querySelector('.print-btn').addEventListener('click', function(){
    window.print();
});
['genderFilter','unitFilter','rankFilter','categoryFilter'].forEach(function(id){
    document.getElementById(id).addEventListener('change', function(){
        document.forms[0].submit();
    });
});
document.getElementById('genderSearch').addEventListener('input', function() {
    const query = this.value.toLowerCase();
    document.querySelectorAll('#genderTable tbody tr').forEach(function(row) {
