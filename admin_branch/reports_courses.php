...
list($courses, $units, $ranks, $categories) = getCourseOptions($pdo, $filter_unit, $filter_rank, $filter_category, $filter_course);
...
function getStaffCourses($pdo, $service_number) {
    ...
    foreach ($courses as $course) {
        $out[] = htmlspecialchars($course->courseName) .
            ($course->date_completed ? " <small class='text-muted'>(" . htmlspecialchars($course->date_completed) . ")</small>" : "");
    }
...
<div class="content-wrapper with-sidebar">
    <div class="container-fluid">
        <div class="main-content">
            <h1 class="section-title mb-4"><i class="fas fa-graduation-cap"></i> <?= htmlspecialchars($pageTitle) ?></h1>
            <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                <i class="fas fa-question-circle me-2"></i>
                <span>
                    Filter by course, rank, unit, category. Use the search box for instant filtering. Export, print, show/hide columns. Double-click row for details.
                </span>
            ...
            <form class="row g-3 mb-4" method="get" action="">
                <div class="col-md-2">
                    <select name="courseID" id="courseFilter" class="form-select">
                        ...
                        <?php foreach ($courses as $c): ?>
                            <option value="<?= $c->id ?>" <?= ($filter_course == $c->id) ? 'selected':''?>><?= htmlspecialchars($c->name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-3">
                    <input type="text" id="coursesSearch" name="search" class="form-control" placeholder="Quick Search..." value="<?=htmlspecialchars($search)?>">
                </div>
                <div class="col-md-1">
            ...
            <div class="mb-2">
                ...
                if ($filter_course !== '') {
                    $columns['courseName'] = 'Course';
                    $columns['date_completed'] = 'Date Completed';
                }
            ...
            <div class="table-responsive print-friendly">
                <table class="table table-bordered table-hover align-middle" id="coursesTable">
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('coursesTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"Courses"});
...
document.getElementById('exportPDFBtn').addEventListener('click', function(){
    ...
    let doc = new jsPDF();
    let startY = 20;
    doc.text("Courses Report", 14, startY);
...
['courseFilter','unitFilter','rankFilter','categoryFilter'].forEach(function(id){
    document.getElementById(id).addEventListener('change', function(){
        document.forms[0].submit();
    });
});
document.getElementById('coursesSearch').addEventListener('input', function() {
    const query = this.value.toLowerCase();
    document.querySelectorAll('#coursesTable tbody tr').forEach(function(row) {
