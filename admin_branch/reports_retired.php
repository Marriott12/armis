...
function getRetiredOptions($pdo, $unit, $rank, $cat) {
    ...
    $unitSql = "SELECT DISTINCT u.id, u.name FROM units u JOIN staff s ON s.unit_id = u.id WHERE s.svcStatus = 'Retired'";
    $rankSql = "SELECT DISTINCT r.id, r.name FROM ranks r JOIN staff s ON s.rank_id = r.id WHERE s.svcStatus = 'Retired'";
...
if ($search !== '') {
    ...
    $count_sql .= " AND (s.appt LIKE '%$search_esc%' OR s.service_number LIKE '%$search_esc%' OR s.last_name LIKE '%$search_esc%' OR s.first_name LIKE '%$search_esc%' OR r.name LIKE '%$search_esc%' OR u.name LIKE '%$search_esc%' OR s.category LIKE '%$search_esc%')";
}
