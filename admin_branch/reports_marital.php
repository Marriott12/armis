...
$pageTitle = "Marital Status Report as at " . date('d-M-Y');
$currentPage = "reports";
...
function getMaritalOptions($pdo, $unit, $rank, $cat) {
    $maritalSql = "SELECT DISTINCT marital FROM staff WHERE marital IS NOT NULL AND marital <> '' AND svcStatus = 'Active'";
    ...
    return [
        $pdo->query($maritalSql)->fetchAll(PDO::FETCH_COLUMN),
        fetchAll($unitSql . " ORDER BY u.name ASC"),
        fetchAll($rankSql . " ORDER BY r.name ASC"),
        fetchAll($catSql . " ORDER BY s.category ASC")
    ];
}
$filter_marital = $_GET['marital'] ?? '';
$filter_unit = $_GET['unitID'] ?? '';
$filter_rank = $_GET['rankID'] ?? '';
$filter_category = $_GET['category'] ?? '';
$search = trim($_GET['search'] ?? '');

list($maritals, $units, $ranks, $categories) = getMaritalOptions($pdo, $filter_unit, $filter_rank, $filter_category);
...
if ($filter_marital !== '')   { $sql .= " AND s.marital_status = ?"; $params[] = $filter_marital; }
...
if ($search !== '') {
    $sql .= " AND (s.marital_status LIKE ? OR s.service_number LIKE ? OR s.last_name LIKE ? OR s.first_name LIKE ? OR r.name LIKE ? OR u.name LIKE ? OR s.category LIKE ?)";
...
<div class="content-wrapper with-sidebar">
    <div class="container-fluid">
        <div class="main-content">
            <h1 class="section-title mb-4"><i class="fas fa-heart"></i> <?= htmlspecialchars($pageTitle) ?></h1>
            <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                <i class="fas fa-question-circle me-2"></i>
                <span>
                    Filter by marital status, unit, rank, category. Use the search box for instant filtering. Export, print, show/hide columns. Double-click row for details.
                </span>
            ...
            <form class="row g-3 mb-4" method="get" action="">
                <div class="col-md-2">
                    <select name="marital" id="maritalFilter" class="form-select">
                        <option value="">Marital Status</option>
                        <?php foreach ($maritals as $m): ?>
                            <option value="<?= htmlspecialchars($m) ?>" <?= ($filter_marital == $m) ? 'selected':''?>><?= htmlspecialchars($m) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-3">
                    <input type="text" id="maritalSearch" name="search" class="form-control" placeholder="Quick Search..." value="<?=htmlspecialchars($search)?>">
                </div>
                <div class="col-md-1">
            ...
            <div class="mb-2">
                ...
                <?php $columns = [
                    'marital_status'=>'Marital Status','unit'=>'Unit','rank'=>'Rank','service_number'=>'Service No','surname'=>'Surname','first_name'=>'First Name(s)',
            ...
            <div class="table-responsive print-friendly">
                <table class="table table-bordered table-hover align-middle" id="maritalTable">
                    ...
                    <tbody>
                        ...
                        <?php else: $i=1; foreach($staff as $s): ?>
                            <tr ondblclick="alert('Audit/History details coming soon.')">
                                <td><?= $i++ ?></td>
                                <td class="col-marital_status"><?= htmlspecialchars($s->marital ?? '') ?></td>
            ...
            <div class="d-flex justify-content-center my-3">
                <nav aria-label="Marital Status pagination">
...
document.getElementById('exportCSVBtn').addEventListener('click', function() {
    let table = document.getElementById('maritalTable');
    ...
    link.download = 'marital_report.csv';
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('maritalTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"Marital"});
    XLSX.writeFile(wb, 'marital_report.xlsx');
});
document.getElementById('exportPDFBtn').addEventListener('click', function(){
    ...
    let doc = new jsPDF();
    let startY = 20;
    doc.text("Marital Report", 14, startY);
    rows.forEach(function(row, idx){
        doc.text(row.join(" | "), 14, startY + 8 + idx*8);
    });
    doc.save("marital_report.pdf");
});
document.querySelector('.print-btn').addEventListener('click', function(){
    window.print();
});
['maritalFilter','unitFilter','rankFilter','categoryFilter'].forEach(function(id){
    document.getElementById(id).addEventListener('change', function(){
        document.forms[0].submit();
    });
});
document.getElementById('maritalSearch').addEventListener('input', function() {
