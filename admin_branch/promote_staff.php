...
<script>
    ...
    // Initial update if promotion type is already selected
    if ($('#promotion_type').val()) {
        console.log('Promotion type already selected on page load:', $('#promotion_type').val());
        setTimeout(updateNextRank, 500); // Slight delay to ensure page is ready
    }
});

function renderConfirmSummary(selected) {
    const currentRankName = $('#current_rank option:selected').text() || 'Unknown Rank';
    const nextRankName = $('#next_rank_display').val() || 'Unknown Rank';
    const promotionType = $('input[name="promotion_type"]:checked').val() || 'promotion';
    const promotionDate = $('#promotion_date').val() || '';
    
    let summary = `
        <div class="alert alert-info">
            <h6><i class="fa fa-info-circle"></i> Promotion Summary</h6>
            <p><strong>Action:</strong> ${promotionType.charAt(0).toUpperCase() + promotionType.slice(1)}</p>
            <p><strong>From:</strong> ${currentRankName} <strong>To:</strong> ${nextRankName}</p>
            <p><strong>Effective Date:</strong> ${promotionDate}</p>
            <p><strong>Staff Members:</strong> ${selected.length}</p>
        </div>
        <h6>Selected Staff Members:</h6>
    ...
    selected.forEach(svcNo => {
        // Try to get staff details from cache first, then fallback to select option text
        let staff = window.staffDetailsCache && window.staffDetailsCache[svcNo] 
            ? window.staffDetailsCache[svcNo] 
            : null;
        
        let staffName = 'Loading...';
        let currentRank = currentRankName;
        let unit = '-';
        
        if (staff) {
            staffName = `${staff.first_name || ''} ${staff.last_name || ''}`.trim() || staff.full_name || 'Unknown';
            currentRank = staff.rank_name || currentRankName;
            unit = staff.unit_name || staff.unit || '-';
        } else {
            // Fallback to select option text
            let optionText = $(`#selected_staff option[value='${svcNo}']`).text();
            if (optionText && optionText !== svcNo) {
                staffName = optionText.replace(svcNo, '').replace(/[()-]/g, '').trim();
            }
        }
    ...
    summary += `
        ...
        <div class="alert alert-warning mt-3">
            ...
            <strong>Warning:</strong> This action will permanently update the rank records for ${selected.length} staff member(s). 
...
// Initialize the profile modal functionality
function initProfileModal() {
    // Single unified click handler for staff profile links
    $(document).off('click', '.profile-trigger').on('click', '.profile-trigger', function(e) {
        e.preventDefault();
        const svcNo = $(this).data('svcno');
        console.log('Profile trigger clicked for service number:', svcNo);
        
        if (typeof window.showStaffProfile === 'function') {
            window.showStaffProfile(svcNo);
        } else if (typeof showStaffProfile === 'function') {
            showStaffProfile(svcNo);
        } else {
            console.error('No showStaffProfile function available');
        }
    });
    
    // Make sure the close button works
    $(document).on('click', '#closeProfileModal', function() {
        var modalEl = document.getElementById('staffProfileModal');
...
// Function to show staff profile in modal
// Note: Duplicate initFormHandlers function removed - using proper one in script initialization section
    // DISABLED: Orphaned code removed - duplicate event handlers cause conflicts
    /*
        const rankId = $(this).val();
        if (rankId) {
            // Show preparation message immediately
            $('#search_debug').removeClass('d-none')
                .html('<div class="alert alert-info mt-2 p-2">� Rank selected. Setting up staff search...</div>');
    ...
    // Promotion type change handler - handled by external promote_staff.js
...
function renderStaffPanels(selected) {
                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col-md-3 mb-2 profile-trigger" style="cursor:pointer;" data-svcno="${svcNo}">
                            <strong>${staff.text || svcNo}</strong>
                            <br>
                        ...
                        <div class="col-md-3 mb-2">
                            ...
                            <input type="text" name="promotion_authority[${svcNo}]" class="form-control authority-input" aria-label="Authority for ${staff.text || svcNo}" required>
                        </div>
                        <div class="col-md-3 mb-2">
                            <label class="form-label mb-0">Remark</label>
                            <input type="text" name="promotion_remark[${svcNo}]" class="form-control remark-input" aria-label="Remark for ${staff.text || svcNo}">
                        </div>
                        <div class="col-md-3 mb-2">
    ...
    // Set up profile trigger handlers - removed duplicate, handled by initProfileModal()
    
    enablePromoteButton();
    $('.authority-input').on('input', enablePromoteButton);
}

// Enable/disable promote button based on form validation
function enablePromoteButton() {
    console.log('enablePromoteButton called');
    let allFilled = true;
    let hasStaff = $('#selected_staff').val() && $('#selected_staff').val().length > 0;
    let hasPromotionType = $('#promotion_type').val() !== '';
    let hasNextRank = $('#next_rank').val() !== '';
    let hasDate = $('#promotion_date').val() !== '';
    
    $('.authority-input').each(function() { 
        if (!$(this).val()) allFilled = false; 
    });
    
    let isValid = allFilled && hasStaff && hasPromotionType && hasNextRank && hasDate;
    console.log('Form validation status:', {
        allAuthoritiesFilled: allFilled,
        hasStaff: hasStaff,
    ...
    $('#showConfirmModal').prop('disabled', !isValid);
}

// Main initialization on document ready
$(document).ready(function() {
    console.log('Document ready - initializing promotion functionality');
    
    // Initialize the staff profile modal functionality
    initProfileModal();
    
    // Initialize form handlers
    initFormHandlers();
    
    // Add bulk confirm button handler
    $('#bulkConfirmSubmitBtn').on('click', function() {
        $('#promotionForm').submit();
    });
    
    // Add show confirmation modal handler
...
function validatePromotionForm() {
    ...
    // Check selected staff
        if (!$('#selected_staff').val() || $('#selected_staff').val().length === 0) {
        ...
        if (!$('#promotion_type').val()) {
            alert('Please select a promotion/reversion type.');
        ...
        // Check next rank
        if (!$('#next_rank').val()) {
            alert('Please select a next/previous rank.');
            $('#next_rank').focus();
            isValid = false;
            return isValid;
        }
        ...
        // Check date
        if (!$('#promotion_date').val()) {
            alert('Please select a promotion/reversion date.');
            $('#promotion_date').focus();
            isValid = false;
            return isValid;
        }
        
        // Check all authority fields
        $('.authority-input').each(function() {
            if (!$(this).val()) {
                alert('All authority fields are required.');
                $(this).focus();
                isValid = false;
    ...
    // Helper function to update next rank
function updateNextRank() {
    const promotionType = $('#promotion_type').val();
    console.log('updateNextRank called with promotion type:', promotionType);
    
    if (!promotionType) {
        console.log('No promotion type selected, skipping next rank update');
        $('#next_rank_display').val('');
        $('#next_rank').val('');
        return;
    }
    
    // Use the external JS module function if available
    if (typeof window.updateNextRankBasedOnType === 'function') {
        console.log('Using external JS module for next rank calculation');
        window.updateNextRankBasedOnType(promotionType.toLowerCase());
    } else {
        console.warn('External JS module not available, using fallback');
        $('#next_rank_display').val('Loading external module...');
        
        // Try again after a short delay to allow module to load
        setTimeout(() => {
            if (typeof window.updateNextRankBasedOnType === 'function') {
                console.log('External JS module now available, retrying');
                window.updateNextRankBasedOnType(promotionType.toLowerCase());
            } else {
                console.error('External JS module still not available after delay');
                $('#next_rank_display').val('Module not loaded - please refresh page');
            }
        }, 1000);
    }
    
    // No need to trigger promotion type change here, it's already handled in the change event
}

// Fallback function to get the next rank if AJAX fails
function fallbackRankUpdate(currentRankId, promotionType) {
    console.log('Using fallback rank update method for current rank ID:', currentRankId, 'and promotion type:', promotionType);
    
    // Get rank details from the page if available
    const currentRankName = <?= json_encode($currentRankName ?? '') ?>;
    const rankCategory = <?= json_encode($category ?? '') ?>;
    
    if (rankCategory) {
        console.log('Category from page:', rankCategory);
        // Based on the promotion type, show an appropriate message
        let actionType = promotionType === 'promotion' ? 'higher' : 'lower';
        let message = `Please select a ${actionType} rank manually. The current rank is ${currentRankName} in the ${rankCategory} category.`;
        
        $('#next_rank_display').val(`Select ${actionType} rank manually`);
        $('#rankUpdateError').remove();
        $('<div id="rankUpdateError" class="alert alert-info mt-2">' + message + '</div>')
            .insertAfter('#next_rank_display');
        
        enablePromoteButton();
    }
}

// Function to show staff profile
function showStaffProfile(serviceNumber) {
    if (!serviceNumber) {
        console.error('No service number provided for profile');
        return;
    }
    
    console.log('Loading profile for service number:', serviceNumber);
    
    try {
        // Show loading in modal
        $('#staffProfileContent').html('<div class="text-center p-4"><i class="fas fa-spinner fa-spin fa-2x"></i><p class="mt-2">Loading profile data...</p></div>');
        
        // Show the modal
        var profileModal = document.getElementById('staffProfileModal');
        if (!profileModal) {
        ...
        // Load the profile data via AJAX
        $.ajax({
            url: 'ajax_get_staff_profile.php',
            type: 'GET',
            data: { service_number: serviceNumber },
            dataType: 'html',
            success: function(response) {
                console.log('Profile data loaded successfully');
                $('#staffProfileContent').html(response);
            },
            error: function(xhr, status, error) {
                console.error('Error loading profile:', error);
                console.log('Response status:', status);
                console.log('Response text:', xhr.responseText);
                $('#staffProfileContent').html('<div class="alert alert-danger">Error loading profile: ' + error + '</div>');
            }
        });
    } catch (e) {
        console.error('Error showing profile modal:', e);
        alert('Error showing profile: ' + e.message);
    }
}

// Render staff detail panels with proper information
function renderStaffPanels(selected) {
    const panel = $('#staffDetailsPanel');
    panel.empty();
            ...
            <div class="card mb-2 staff-detail-card" data-svcno="${svcNo}">
                <div class="card-body">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-3 mb-2">
                            <label class="form-label mb-0">Authority <span class="text-danger">*</span></label>
                            <input type="text" name="promotion_authority[${svcNo}]" class="form-control authority-input" aria-label="Authority for ${staffText}" required>
                        </div>
                        <div class="col-md-3 mb-2">
                            <label class="form-label mb-0">Remark</label>
                                ...
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-md-3 mb-2">
                                                <span class="badge bg-info">Current Unit: <span class="current-unit">\${$('#selected_staff option[value="' + svcNo + '"]').text().split('(')[1]?.replace(')', '') || ''}</span></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
