...
function getTradeOptions($pdo, $unit, $rank, $cat) {
    ...
    // Add placeholder if any empty trades exist
    if (in_array('', $trades, true)) {
        // Remove empty string and add placeholder
        $trades = array_diff($trades, ['']);
        $trades[] = '__NO_TRADE_ASSIGNED__';
    }
    return [
        $trades,
...
if ($filter_trade === '__NO_TRADE_ASSIGNED__') {
    $filter_trade = '';
    $filter_no_trade = true;
} else {
    $filter_no_trade = false;
}
...
<div class="content-wrapper with-sidebar">
    <div class="container-fluid">
        <div class="main-content">
            <h1 class="section-title mb-4"><i class="fas fa-toolbox"></i> <?= htmlspecialchars($pageTitle) ?></h1>
            <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                <i class="fas fa-question-circle me-2"></i>
                <span>
                    Filter by trade, unit, rank, category. Use the search box for instant filtering. Export, print, show/hide columns. Double-click row for details.
                </span>
            ...
            <form class="row g-3 mb-4" method="get" action="">
                <div class="col-md-2">
                    <select name="trade" id="tradeFilter" class="form-select">
                        <option value="">Trade</option>
                        <?php foreach ($trades as $t): ?>
                            <?php if ($t === '__NO_TRADE_ASSIGNED__'): ?>
                                <option value="__NO_TRADE_ASSIGNED__" <?= (isset($_GET['trade']) && $_GET['trade'] === '__NO_TRADE_ASSIGNED__') ? 'selected':''?>>No Trade Assigned</option>
                            <?php else: ?>
                                <option value="<?= htmlspecialchars($t) ?>" <?= (isset($_GET['trade']) && $_GET['trade'] === $t) ? 'selected':''?>><?= $t === '' ? 'No Trade Assigned' : htmlspecialchars($t) ?></option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    <input type="text" id="tradeSearch" name="search" class="form-control" placeholder="Quick Search..." value="<?=htmlspecialchars($search)?>">
                </div>
                <div class="col-md-1">
            ...
            <div class="table-responsive print-friendly">
                <table class="table table-bordered table-hover align-middle" id="tradeTable">
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('tradeTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"Trade"});
