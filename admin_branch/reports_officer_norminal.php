...
function getDynamicOptions($pdo, $selectedRank, $selectedUnit, $selectedCategory) {
    $rankSql = "SELECT DISTINCT r.rankId as id, COALESCE(r.rankId, r.rankId) as name FROM `rank` r JOIN staff s ON s.rankId = r.rankId WHERE s.svcStatus = 'Active'";
        $unitSql = "SELECT DISTINCT u.unitId as id, u.unitId as name FROM unit u JOIN staff s ON s.unitId = u.unitId JOIN `rank` r ON s.rankId = r.rankId WHERE s.svcStatus = 'Active'";
...
if ($reportType === 'officer') {
    $sql .= " AND r.rankIndex >= 1 AND r.rankIndex <= 13";
    $count_sql .= " AND r.rankIndex >= 1 AND r.rankIndex <= 13";
} elseif ($reportType === 'nco') {
    $sql .= " AND r.rankIndex >= 15 AND r.rankIndex <= 26";
    $count_sql .= " AND r.rankIndex >= 15 AND r.rankIndex <= 26";
} elseif ($reportType === 'ce') {
    $sql .= " AND r.rankIndex = 28";
    $count_sql .= " AND r.rankIndex = 28";
}
$count_params = [];
