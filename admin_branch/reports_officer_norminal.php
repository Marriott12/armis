...
$pageTitle = "Officer Nominal Roll as at " . date('d-M-Y');
$moduleName = "Admin Branch";
$moduleIcon = "users-cog";
$currentPage = "officer_norminal";
...
function getDynamicOptions($pdo, $selectedRank, $selectedUnit, $selectedCategory) {
    $rankSql = "SELECT DISTINCT r.rankId as id, COALESCE(r.rankId, r.rankId) as name FROM `rank` r JOIN staff s ON s.rankId = r.rankId WHERE s.svcStatus = 'Active'";
        $unitSql = "SELECT DISTINCT u.unitId as id, u.code as name FROM unit u JOIN staff s ON s.unitId = u.unitId JOIN `rank` r ON s.rankId = r.rankId WHERE s.svcStatus = 'Active'";
...
if ($search !== '') {
    $sql .= " AND (s.svcNo LIKE ? OR s.lName LIKE ? OR s.fName LIKE ? OR r.rankId LIKE ? OR r.rankId LIKE ? OR u.code LIKE ? OR s.category LIKE ? OR s.svcStatus LIKE ? OR s.DOB LIKE ? OR s.attestDate LIKE ?)";
    $count_sql .= " AND (s.svcNo LIKE ? OR s.lName LIKE ? OR s.fName LIKE ? OR r.rankId LIKE ? OR r.rankId LIKE ? OR u.code LIKE ? OR s.category LIKE ? OR s.svcStatus LIKE ? OR s.DOB LIKE ? OR s.attestDate LIKE ?)";
    for ($i = 0; $i < 10; $i++) {
        $params[] = "%$search%";
        $count_params[] = "%$search%";
    }
...
if ($sort_col && array_key_exists($sort_col, $sortable_columns)) {
    ...
    $sql .= " ORDER BY s.attestDate ASC, s.svcNo ASC";
}
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="helpModalLabel">Officer Nominal Roll Help</h5>
...
document.getElementById('exportCSVBtn').addEventListener('click', function() {
    ...
    link.download = 'officer_nominal_report.csv';
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('seniorityTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"Officer Nominal"});
    XLSX.writeFile(wb, 'officer_nominal_report.xlsx');
});
...
document.getElementById('exportPDFBtn').addEventListener('click', function(){
    ...
    let doc = new jsPDF();
    let startY = 20;
    doc.text("Officer Nominal Report", 14, startY);
    rows.forEach(function(row, idx){
        doc.text(row.join(" | "), 14, startY + 8 + idx*8);
    });
    doc.save("officer_norminal_report.pdf");
});
