...
$pageTitle = "CE Nominal Roll as at " . date('d-M-Y');
$moduleName = "Admin Branch";
$moduleIcon = "users-cog";
$currentPage = "ce_nominal";

// Determine report type (officer, nco, ce)
$reportType = $_GET['report_type'] ?? 'ce';
                ...
                <table class="table table-bordered table-hover align-middle" id="seniorityTable">
                    ...
                    <tbody>
                        <?php if (count($staff) == 0): ?>
                            <tr><td colspan="<?= count($columns)+2 ?>" class="text-center text-muted">No CEs found.</td></tr>
...
document.getElementById('exportCSVBtn').addEventListener('click', function() {
    ...
    link.download = 'ce_nominal_report.csv';
...
document.getElementById('exportExcelBtn').addEventListener('click', function() {
    let table = document.getElementById('seniorityTable');
    let wb = XLSX.utils.table_to_book(table, {sheet:"CE Nominal"});
    XLSX.writeFile(wb, 'ce_nominal_report.xlsx');
});
...
document.getElementById('exportPDFBtn').addEventListener('click', function(){
    ...
    let doc = new jsPDF();
    let startY = 20;
    doc.text("CE Nominal Report", 14, startY);
    rows.forEach(function(row, idx){
        doc.text(row.join(" | "), 14, startY + 8 + idx*8);
    });
    doc.save("ce_norminal_report.pdf");
});
