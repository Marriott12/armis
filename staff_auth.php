...
// Replaces UserSpice functionality with Staff table authentication
...
// Database connection using existing UserSpice config
function getDatabase() {
    ...
    // Fallback to create new connection using config
    global $GLOBALS;
...
function loginUser($username, $password) {
    try {
        ...
        $_SESSION['user'] = [
            ...
            'email' => $staff['email'],
            'role' => $staff['role'],
            'must_reset_password' => (bool)$staff['must_reset_password'],
            'logged_in' => true,
            'login_time' => time(),
            'last_activity' => time()
        ];
...
function mustResetPassword() {
    $user = getCurrentUser();
    return $user && $user['must_reset_password'];
}
