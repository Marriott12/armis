...
function getMissionHistory($missionId) {
    $stmt = $db->prepare('SELECT h.*, s.username FROM operations_mission_history h LEFT JOIN staff s ON h.changed_by = s.id WHERE h.mission_id = ? ORDER BY h.changed_at DESC');
