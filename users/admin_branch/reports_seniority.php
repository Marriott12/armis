...
// Fetch ranks, units, and categories for filter dropdowns
...
if ($search !== '') {
    $sql .= " AND (s.svcNo LIKE ? OR s.lname LIKE ? OR s.fname LIKE ? OR r.rankName LIKE ? OR u.unitName LIKE ? OR s.category LIKE ? OR s.svcStatus LIKE ? OR s.DOB LIKE ? OR s.attestDate LIKE ?)";
    for ($i = 0; $i < 9; $i++) {
        $params[] = "%$search%";
    }
}
...
<div class="container my-5">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between">
            <h4 class="mb-0"><i class="fa fa-list-ol"></i> Seniority Roll</h4>
            <span class="badge bg-light text-primary fs-6"><?= count($staff) ?> Staff Listed</span>
        </div>
        <div class="card-body">
            <form class="row g-3 mb-4" method="get" action="">
                <div class="col-md-3">
                    <input type="text" id="senioritySearch" name="search" class="form-control" placeholder="Type to filter..." value="<?=htmlspecialchars($search)?>">
                </div>
                <div class="col-md-2">
...
document.getElementById('senioritySearch').addEventListener('keyup', function() {
